...
const kumo_temp_table_1 = require("./kumo-temp-table");
...
class KumoThermostatAccessory {
    ...
    async setTargetTemperature(value) {
        ...
        this.platform.log.info(`[TEMP CHANGE] ${this.accessory.displayName}: HomeKit sent ${temp.toFixed(3)}°C `
            + `(${(0, kumo_temp_table_1.kumoFahrenheit)(kumoTemp)}°F) -> Kumo ${kumoTemp}°C`);
        if (!this.currentStatus) {
    ...
    correctTemp(celsius) {
        return this.useFahrenheitCorrection ? (0, kumo_temp_table_1.toHomeKitCelsius)(celsius) : celsius;
    }
    toKumoTemp(celsius) {
        return this.useFahrenheitCorrection ? (0, kumo_temp_table_1.toKumoCelsius)(celsius) : celsius;
    }
    ...
    async setThresholdTemperature(field, temp) {
        ...
        const kumoTemp = this.toKumoTemp(temp);
        this.platform.log.info(`[${label}] ${this.accessory.displayName}: HomeKit sent ${temp.toFixed(1)}°C `
            + `(${(0, kumo_temp_table_1.kumoFahrenheit)(kumoTemp)}°F) -> Kumo ${kumoTemp}°C`);
        if (!this.currentStatus) {
